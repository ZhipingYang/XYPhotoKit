...
#import "XYPhotoTableCell.h"
#import "XYPhotoSelectedAssetManager.h"
#import "XYPhotoDataCategory.h"

@interface XYPhotoTableDataSource () <PHPhotoLibraryChangeObserver, UITableViewDataSource, UITableViewDelegate>
...
@implementation XYPhotoTableDataSource
...
- (instancetype)initWithTableView:(UITableView *)tableView
{
    if (self = [super init]) {
        ...
        [_tableView registerClass:[XYPhotoTableCell class] forCellReuseIdentifier:XYPhotoTableCellReuseIdentifier];
        ...
        [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status) {
            if (status == PHAuthorizationStatusAuthorized) {
                ...
                self.collectionFetchResults = @[albums, topLevelUserCollections];
...
- (void)updateCollectionArrays
{
    ...
    [albums enumerateObjectsUsingBlock:^(PHAssetCollection *obj, NSUInteger idx, BOOL *stop) {
        PHFetchResult *assetsInCollection = [PHFetchResult xy_fetchResultWithAssetCollection:obj mediaType:[XYPhotoSelectedAssetManager sharedManager].mediaType];
    ...
    [topLevelUserCollections enumerateObjectsUsingBlock:^(PHCollection *obj, NSUInteger idx, BOOL *stop) {
		if ([obj isKindOfClass:[PHAssetCollection class]]) {
			PHFetchResult *assetsInCollection = [PHFetchResult xy_fetchResultWithAssetCollection:(PHAssetCollection *)obj mediaType:[XYPhotoSelectedAssetManager sharedManager].mediaType];
			if (assetsInCollection.count > 0 && assetsInCollection.count < NSNotFound) {
...
{
    XYPhotoTableCell *cell = [tableView dequeueReusableCellWithIdentifier:XYPhotoTableCellReuseIdentifier forIndexPath:indexPath];
