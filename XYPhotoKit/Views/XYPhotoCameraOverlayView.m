	...
	[PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status) {
        dispatch_async(dispatch_get_main_queue(), ^{
			if (status == PHAuthorizationStatusAuthorized) {
                self.bottomPreview = [[XYPhotoSelectedAssetPreviewView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, 60)];
				self.bottomPreview.hideControls = YES;
				...
				self.selectedScriptLabel = [[UILabel alloc] initWithFrame:CGRectMake(8, CGRectGetMaxY(self.bottomPreview.frame), self.frame.size.width-8*2, 14)];
