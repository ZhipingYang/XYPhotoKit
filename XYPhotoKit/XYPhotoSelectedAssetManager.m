...
@interface XYPhotoSelectedAssetManager ()

@property (nonatomic) NSMutableArray *selectedAssetsMutableArray;

@end

@implementation XYPhotoSelectedAssetManager

+ (instancetype)sharedManager
{
    static XYPhotoSelectedAssetManager *SharedManager = nil;
...
- (void)resetManager
{
    ...
    self.assetCollectionViewColumnCount = XYPhotoCollectionFlowLayoutDefaultColumns;
...
- (BOOL)addSelectedAsset:(PHAsset *)asset
{
    if ([self assetIsInMediaType:asset] && [self canAddMoreAssets]) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:XYPhotoMultiImagePickerNotifications.assetsChanged object:asset];
        return YES;
    }
    return NO;
}
...
- (void)resetSelectedAsset:(NSArray <PHAsset *>*)assets
{
	...
	[[NSNotificationCenter defaultCenter] postNotificationName:XYPhotoMultiImagePickerNotifications.assetsChanged object:nil];
}
