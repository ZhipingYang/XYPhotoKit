...
@interface CKPhotoCollectionViewCell ()
...
@implementation CKPhotoCollectionViewCell
...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
		...
		if ([CKPhotoSelectedAssetManager sharedManager].maxNumberOfAssets != 1) {
...
- (void)setAsset:(PHAsset *)asset atIndexPath:(NSIndexPath *)indexPath inCollectionView:(UICollectionView *)collectionView withCacheManager:(PHCachingImageManager *)cacheManager
{
	...
	_markIconButon.selected = [[CKPhotoSelectedAssetManager sharedManager].selectedAssets containsObject:asset];
