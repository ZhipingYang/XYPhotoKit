//
//  CKPhotoSelectedAssetPreviewView..m
//  XYPhotoKitDemo
//
...
#import "CKPhotoBottomPreviewCell.h"
#import "CKPhotoKitHelper.h"
#import "CKPhotoDataCategory.h"

@interface CKPhotoSelectedAssetPreviewView ()<UICollectionViewDelegate, UICollectionViewDataSource>
{
...
@implementation CKPhotoSelectedAssetPreviewView
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
		...
		// 用于初始化
		self.assetArray = [CKPhotoSelectedAssetManager sharedManager].selectedAssets;
		// 用于调整collection的cell布局（意义性不大，本可以忽略但是这边要保留，等同于`setAssetArray:`**不要去除这片脏代码**）
		dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
			_assetArray = [CKPhotoSelectedAssetManager sharedManager].selectedAssets;
...
- (void)initBaseViews
{
	...
	CKPhotoCollectionFlowLayout *layout = [[CKPhotoCollectionFlowLayout alloc] init];
	layout.scrollDirection = UICollectionViewScrollDirectionHorizontal;
	
	// 预览已选择的assets
	self.collectionView = [[UICollectionView alloc] initWithFrame:self.bounds collectionViewLayout:layout];
	[self.collectionView registerClass:[CKPhotoBottomPreviewCell class] forCellWithReuseIdentifier:NSStringFromClass([CKPhotoBottomPreviewCell class])];
	...
	[self.doneButton addTarget:[CKPhotoSelectedAssetManager sharedManager].delegate action:@selector(doneSelectingAssets) forControlEvents:UIControlEventTouchUpInside];
...
{
	if ([[CKPhotoSelectedAssetManager sharedManager].selectedAssets containsObject:asset]) {
		NSInteger index = [[CKPhotoSelectedAssetManager sharedManager].selectedAssets indexOfObject:asset];
...
{
	CKPhotoBottomPreviewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([CKPhotoBottomPreviewCell class]) forIndexPath:indexPath];
	CKPhotoSelectedAssetManager *manager = [CKPhotoSelectedAssetManager sharedManager];
