...
@interface CKPhotoPreviewOverlayView()<CKPhotoSelectedAssetPreviewViewDelegate>
{
...
@property (nonatomic, strong) CKPhotoSelectedAssetPreviewView *previewView;

@end

@implementation CKPhotoPreviewOverlayView
...
- (instancetype)initWithFrame:(CGRect)frame
{
	...
	if (self) {
		...
		_previewView = [[CKPhotoSelectedAssetPreviewView alloc] initWithFrame:CGRectMake(0, self.frame.size.height-60, self.frame.size.width, 60)];
...
- (void)setDelegate:(id<CKPhotoPreviewOverlayViewDelegate>)delegate
{
	_delegate = delegate;
...
- (void)selectedStateChanged
{
	if (_rightButton.selected) {
		[[CKPhotoSelectedAssetManager sharedManager] removeSelectedAsset:_asset];
		_rightButton.selected = NO;
	} else if ([[CKPhotoSelectedAssetManager sharedManager] addSelectedAsset:_asset]) {
...
{
	_asset = asset;
	_rightButton.selected = [[CKPhotoSelectedAssetManager sharedManager].selectedAssets containsObject:asset];
